...
router.post("/getdevicecredentials", async (req, res) => {
    ...
    const subscribe = userId + "/" + dId + "/+/actdata"
    ...
    setTimeout(() => {
        ...
        console.log("Device Credentials Update")
    }, 15000);
...
router.post('/alarm-webhook', async (req, res) => {
    ...
    try {
        ...
        if (lastNotif == 0) {
            console.log("FIRST THE ALARM")
            saveNotifToMongo(incomingAlarm)
            sendMqttNotif(incomingAlarm)

        }else {
            // RECUPERO EL TIEMPO Y LO PASO A MINUTOS
...
//UPDATE NOTIFOCATION (readed status)
...
async function getDeviceMqttCredentials(dId, userId) {
    try {
        ...
        if(rule.length == 0) {
            ...
            //Se crea la Nueva Regla en mongo
...
function startMqttClient() {
    ...
    const options = {
        ...
        clientId: 'webhook-superuser' + Math.round(Math.random() * (0 - 1000) * -1),
    ...
    client = mqtt.connect ('mqtt://' + process.env.EMQX_NODE_HOST1, options)
