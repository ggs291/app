    ...
    try {
        ...
        if (res.status === 200){
            if (size == 0) {
                ...
                console.log("****** Creating emqx webhook resourses *****".green)
                createResourses ()
            }else if (size == 2){
...
//Create resourses
async function createResourses(){
    ...
    try {
        ...
        if (res1.status === 200){
            console.log("Saver resources created".green)
        }

        const res2 = await axios.post(url, data2, auth)
        if (res2.status === 200){
            console.log("Alarm resources created".green)
        }
