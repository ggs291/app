...
async function getWebUserMqttCredentials(userId){
    ...
    try {
        ...
        const newUserPassword = makeid(10)

        const result = await EmqxAuthRule.updateOne({type:"user", userId: userId}, {$set: {username: newUserName, password: newUserPassword, updatedTime: Date.now()}})
